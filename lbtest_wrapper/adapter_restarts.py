...
class Adapter(object):
    ...
    def _send_testcase_messages(self, commands):
        ...
        for c in commands:
            if c == 'x':
                print_string += self._get_state_sequence(time_out=2) #Need to do this here, otherwise first app will continue to run the rest of the commands before terminating.
                restart_matcher()
                command = self._get_command('')
            else:
        ...
        print_string += self._get_state_sequence(time_out=2)
    ...
    def _get_state_sequence(self, time_out=0.1):
        print_string = ""
        timed_out = False
    ...
    def execute_testcase(self, line):
        ...
        print_string = self._send_testcase_messages(commands)
        self._rollback()
